...
The dependencies in this module are used to access the database and to perform CRUD operations on
orders.

The following dependencies are defined in this module:

* `get_order_by_id`: Gets an order by ID. * `get_orders_in_range`: Gets a list of orders, paginated
by offset and limit. * `add_orders`: Creates new orders. * `get_completed_orders`: Gets a list of
completed orders. * `complete_order`: Marks an order as completed.
"""
...
async def get_orders_in_range(
    ...
    * offset: The offset to start at. * limit: The number of orders to return. * orders_repo: The
    repository that stores the orders.
...
async def add_orders(
    ...
    * create_order_request: The request object containing the orders' details. * orders_repo: The
    repository that stores the orders.
...
async def complete_order(
    ...
    * completed_orders: The list of orders to mark as completed. * orders_repo: The repository that
    stores the orders.
